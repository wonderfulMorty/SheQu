...
<body class="layui-unselect lau-sign-body" style="padding-top: 0px " >
    <form  class="layui-form">
        <div>
            <label class="layui-form-label"><i class="layui-icon layui-icon-password"></i> 选项内容</label>
            <div class="layui-input-block">
                <input type="text" name="moption" placeholder="请输入选项内容" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div>
        ...
        <div class="layui-form-item lau-sign-other" style="margin-top: 20px;text-align:center">
            <button type="button" class="layui-btn layui-btn-normal" lay-submit="" lay-filter="updateChoice" style="margin-right: 100px">提交</button>
        </div>
    </form>
</body>
<script>
    ...
    layui.use(['upload','form'], function(){
        var form=layui.form;
        form.on('submit(updateChoice)', function(result) {
            console.log("result.field:"+typeof (result.field))//object
            res = result.field
            res.id = id;
            res.q_id = question_id
            data = JSON.stringify(res)//把js对象转换成json字符串,string
            console.log("data:"+typeof (data))
            $.ajax({
                url : '${pageContext.request.contextPath}/updateChoiceById',
                data : data,
                type:'POST',
                dataType:'text',

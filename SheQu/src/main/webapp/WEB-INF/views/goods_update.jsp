...
<form  class="layui-form">
    <div>
        <label class="layui-form-label"><i class="layui-icon layui-icon-password"></i> 商品标题</label>
        <div class="layui-input-block">
            <input type="text" name="goods_title" placeholder="请输入商品标题" autocomplete="off" class="layui-input">
        </div>
    </div>
    <div >
        <label class="layui-form-label"><i class="layui-icon layui-icon-password"></i>商品描述</label>
        <div class="layui-input-block">
            <input type="text" name="goods_desc" placeholder="请输入商品描述" autocomplete="off" class="layui-input">
        </div>
    </div>
    <div>
        <label class="layui-form-label" ><i class="layui-icon layui-icon-password"></i>商品图片</label>
    ...
    <div >
        <label class="layui-form-label"><i class="layui-icon layui-icon-password"></i>商品价格</label>
        <div class="layui-input-block">
            <input type="text" name="goods_price" placeholder="请输入商品价格" autocomplete="off" class="layui-input">
        </div>
    </div>
    <div>
        <label class="layui-form-label">选择发布人：</label>
        <div class="layui-input-block">
    ...
    <div class="layui-form-item lau-sign-other" style="margin-top: 20px;text-align:center">
        <button type="button" class="layui-btn layui-btn-normal" lay-submit="" lay-filter="updateGoods" style="margin-right: 100px">提交</button>
    </div>
</form>
</body>
<script>
    ...
    layui.use(['upload','form'], function(){
        ...
        form.on('submit(updateGoods)', function(result) {
            ...
            $.ajax({
                url : '${pageContext.request.contextPath}/updateGoodsById',
                data : data,
                type:'POST',
                dataType:'text',
    ...
    $(function () {
        $.ajax({
            ...
            success: function (data) {
                console.log(data)
                $('#publisher_id').html('')
                $('#publisher_id').append(new Option('请选择发布人账号','请选择发布人账号'))
